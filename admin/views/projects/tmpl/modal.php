...
?>
<form action="<?php echo JRoute::_('index.php?option=com_projectlog&view=projects&layout=modal&tmpl=component&function='.$function);?>" method="post" name="adminForm" id="adminForm" class="form-inline">
	<fieldset class="filter clearfix">
		...
		<div class="filters pull-left">
			...
			<select name="filter_category_id" class="input-medium" onchange="this.form.submit()">
				<option value=""><?php echo JText::_('JOPTION_SELECT_CATEGORY');?></option>
				<?php echo JHtml::_('select.options', JHtml::_('category.options', 'com_projectlog'), 'value', 'text', $this->state->get('filter.category_id'));?>
			</select>
	...
	<table class="table table-striped table-condensed">
		...
		<?php foreach ($this->items as $i => $item) : ?>
			...
			<tr class="row<?php echo $i % 2; ?>">
				<td>
					<a href="javascript:void(0)" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>', '<?php echo $this->escape(addslashes($item->name)); ?>', '<?php echo $this->escape($item->catid); ?>', null, '<?php echo $this->escape(ProjectlogHelperRoute::getProjectRoute($item->id, $item->catid, $item->language)); ?>', '<?php echo $this->escape($lang); ?>', null);">
