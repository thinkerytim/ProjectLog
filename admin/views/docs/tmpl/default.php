...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_projectlog&task=docs.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'docsList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<form action="<?php echo JRoute::_('index.php?option=com_projectlog&view=docs'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php endif;?>
		...
		<?php else : ?>
            <table class="table table-striped" id="docsList">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    foreach ($this->items as $i => $item) :
                        ...
                        $canCreate	= $user->authorise('projectlog.createdoc',     'com_projectlog.project.'.$item->project_id);
                        $canEdit	= $user->authorise('projectlog.editdoc',       'com_projectlog.project.'.$item->project_id);
                        $canCheckin	= $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
                        $canEditOwn	= $user->authorise('projectlog.editdoc.own',   'com_projectlog.project.'.$item->project_id) && $item->created_by == $userId;
                        $canChange	= $user->authorise('projectlog.editdoc.state', 'com_projectlog.project.'.$item->project_id) && $canCheckin;
                        ?>
                        <tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->project_id?>">
                            ...
                            <td class="center">
                                <div class="btn-group">
                                    <?php echo JHtml::_('jgrid.published', $item->published, $i, 'docs.', $canChange, 'cb', $item->publish_up, $item->publish_down); ?>
                                    <?php
                                    // Create dropdown items
                                    $action = $archived ? 'unarchive' : 'archive';
                                    JHtml::_('actionsdropdown.' . $action, 'cb' . $i, 'docs');
                            ...
                            <td class="nowrap has-context">
                                <div class="pull-left">
                                    ...
                                    <?php if ($canEdit || $canEditOwn) : ?>
                                        <a href="<?php echo JRoute::_('index.php?option=com_projectlog&task=doc.edit&id='.(int) $item->id); ?>">
                            ...
                            <td class="hidden-phone">
                                <?php if ($item->association) : ?>
                                    <?php echo JHtml::_('project.docassociation', $item->id); ?>
                                <?php endif; ?>
                            </td>
                            <?php endif;?>
